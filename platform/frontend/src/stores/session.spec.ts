...
describe('session store',()=>{
  ...
  it('logs in admin and always logs out cleanly',()=>{
    ...
    expect(sessionStorage.getItem('armaghan:test20:role')).toBeNull()
    expect(sessionStorage.getItem('armaghan:test20:impersonation')).toBeNull()
  })
  ...
  it('restores explicit admin impersonation inside the same browser session',()=>{
    sessionStorage.setItem('armaghan:test20:role','admin')
    sessionStorage.setItem('armaghan:test20:impersonation','7')
    const store=useSessionStore()
    expect(store.role).toBe('admin')
