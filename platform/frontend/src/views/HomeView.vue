...
const categoryIcons=[Sparkles,Sparkles,Sparkles]
</script>
  ...
  <div class="space-y-8">
    ...
    <section>
      ...
      <div class="grid grid-cols-3 gap-2">
        <RouterLink v-for="(category,index) in categories" :key="category.code" :to="{path:'/products',query:{category:category.code}}" class="overflow-hidden rounded-2xl border border-slate-200 bg-[var(--c-paper)] shadow-sm">
          <SmartImage :src="category.image" :alt="category.name" :label="category.name" />
          <div class="p-3">
            <div class="flex items-center gap-2"><component :is="categoryIcons[index]" :size="16" class="text-[var(--c-primary)]" /><b class="text-sm">{{ category.name }}</b></div>
    ...
    <section>
      ...
      <div class="grid gap-3 md:grid-cols-3">
        <article v-for="item in [
          [Sparkles,'توانمندی‌ها'],
          [BadgeCheck,'اعتبارات و اسناد'],
          [MessageCircleMore,'ارتباط فروش'],
        ]" :key="item[1] as string" class="rounded-2xl border border-slate-200 bg-[var(--c-paper)] p-4 shadow-sm">
          <div class="mb-3 grid h-11 w-11 place-items-center rounded-xl bg-emerald-50 text-emerald-700"><component :is="item[0]" :size="21" /></div>
          <b class="text-sm">{{ item[1] }}</b>
