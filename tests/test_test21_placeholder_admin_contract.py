...
assert "../../t/21" in vite
assert "test21-build" in workflow and "t/21" in workflow
assert "test_test21_placeholder_admin_contract.py" in workflow
assert "20" in immutable
...
assert "armaghan:test21:placeholder-set" in design
...
for key_file in [
    ...
    assert "test21" in key_file.read_text(encoding="utf-8")
