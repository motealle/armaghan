...
assert "../../t/22" in vite or "../../t/23" in vite

assert ("test22-build" in workflow and "path: t/22" in workflow) or ("test23-build" in workflow and "path: t/23" in workflow)
assert 'os.path.isdir("t/22")' in workflow or 'os.path.isdir("t/23")' in workflow
assert 'os.walk("t/22")' in workflow or 'os.walk("t/23")' in workflow
...
assert "Tests 01–22 are frozen; current source target is Test 23." in rules or "Tests 01–21 are frozen; current source target is Test 22." in rules
...
for path in strict_test22_files:
    ...
    assert "armaghan:test22" in text or "armaghan:test23" in text, path
    assert "armaghan:test21" not in text, path

catalog = (SRC / "stores/catalog.ts").read_text(encoding="utf-8")
assert "const KEY='armaghan:test22:products-v1'" in catalog or "const KEY='armaghan:test23:products-v1'" in catalog
assert "'armaghan:test21:products-v1'" in catalog
if "const KEY='armaghan:test23:products-v1'" in catalog:
...
assert "armaghan:test22:role" in session_spec or "armaghan:test23:role" in session_spec
assert "armaghan:test22:impersonation" in session_spec or "armaghan:test23:impersonation" in session_spec
